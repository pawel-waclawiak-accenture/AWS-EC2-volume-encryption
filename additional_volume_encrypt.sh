...
aws ec2 create-volume --availability-zone $availability_zone --tag-specifications "ResourceType=volume,Tags=[{Key=instance,Value=$instance_id},{Key=device,Value=$mounting_point},{Key=Name,Value=$instance_name-secondary-volume}]" --snapshot-id $snapshot_id | tee >(jq -r '.VolumeId' > $path/new_volume_id)
